...
Page({
  ...
  async getVideoGroupListData(){
    ...
    let data = videoGroupListData.data.slice(0,14)
    this.setData({
      videoGroupList:data,
      navId:data[0].id
    })
  ...
  async changeNav(event){
    let navId = event.currentTarget.id//通过id传参，传入的会转换成字符串，需要转换成数字才能在变迁诶比较，判断active
    this.setData({
      //将navId转换成数字
      navId:navId*1,//navId:navId >>> 0
      videoList:[]//切换按钮时，数据还未加载出来，但是先前的视频还在，所以需要清空
    })
  ...
  async getVideoList(id){
    let videoListData = await request('/video/group',{id:id})
    console.log(videoListData)
    //在这里获取到数据后，关闭显示正在加载的函数wx.showLoading
    wx.hideLoading()
    let index = 0
  ...
  handlePlay(event){
    ...
    //创建video标签的正在播放的视频的实例对象videoContext，放到this上
    this.vid = vid
  ...
  onShareAppMessage: function (from) {
    ...
    if(from === 'button'){
      return {
        ...
        imageUrl: '/static/images/video/video.jpg'
      }
    }else {
      return {
        title: '来自menu的转发',
